...
figurewidth = 9; %cm
figure1 = figure('Position',[10 10 figurewidth figurewidth*0.6]*36.36);
axes1 = axes('Parent',figure1);
contour(auc,10)
x=linspace(min(pca_order_list),max(pca_order_list),150);
y=linspace(min(sqenc_list),max(sqenc_list),150);
[X,Y]=meshgrid(x,y);
xx = repmat(pca_order_list,length(sqenc_list),1);
yy = repmat(sqenc_list,length(pca_order_list),1)';
auc_temp=auc';
xxx=xx(:);yyy=yy(:);zzz=auc_temp(:);
F=TriScatteredInterp(xxx,yyy,zzz);
% contourf(X,Y,F(X,Y),50,'LineColor','none')
contourf(Y,X,F(X,Y),50,'LineColor','none')
colorbar('location','northoutside')
% caxis([0.3, 0.86]);
% cc = linspace(0.0,0.7,256)'*[1,1,1];
% colormap(cc)
colormap('bone')

ylabel('Number of PCs retained, {\it{\fontname{Times} k}}')
...
exportgraphics(figure1,'fig.sq_pc_heatmap_owt.eps','Resolution',1000)