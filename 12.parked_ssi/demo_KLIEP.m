...
[wh_x_de,wh_xdisp]=KLIEP2(x_de,x_nu,xdisp,0.1);
SCORE_WW = mean(log(wh_x_de))
...
% legend('p_{de}(x)','p_{nu}(x)', 'Location','northeast')
% xlabel('x')
...
% legend('w(x)','w-hat(x)','w-hat(x^{de})', 'Location','northeast')
% xlabel('x')
% set(gcf,'PaperUnits','centimeters');
% set(gcf,'PaperPosition',[0 0 12 9]);
