...
dataset11 = squeeze(dataset1(120*20+1:end,node,:));
% dataset1 = cat(3,dataset1(1:end/2,:,:),dataset1(end/2+1:end,:,:));
dataset111 = dataset11(:);
dataset_test_1 = dataset111(1:20*60*time);

load('dam_scour0.24m_seed201To250_1e6.mat')
dataset22 = squeeze(dataset2(120*20+1:end,node,:));
% dataset21 = cat(3,dataset2(1:end/2,:,:),dataset2(end/2+1:end,:,:));
dataset222 = dataset22(:);
dataset_test_2 = dataset222(1:20*60*time);
...
legend([p1,p2],'Undamaged state','0.24 m damaged state',...
        'location','northeast','Orientation','vertical')
legend boxoff
set(findall(f,'-property','FontSize'),'FontSize',7)
exportgraphics(f,'fig3.PSD_boundary.eps','Resolution',1000)
