...
load('dam_scour0.36m_seed201To210.mat')
dataset21 = dataset2;
...
%             nla
    ...
    %                     rng(100*(sn-1)+nrr);
                        ...
                        SCORE_SS = zeros(SS,1);
                        for ss = 1:SS
                            X1 = zeros(size(Dataset100,1)-sqenc,10);
                            X2 = zeros(size(Dataset200,1)-sqenc,10);
                            for j=1:(size(Dataset100,1)-sqenc)
                                X1(j,:) = Dataset100(j:5:j+sqenc-1,ss);
                            end
                            for j=1:(size(Dataset200,1)-sqenc)
                                X2(j,:) = Dataset200(j:5:j+sqenc-1,ss);
                            end
                            x_de=X1';
                            x_nu=X2';
