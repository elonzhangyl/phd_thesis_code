...
%             nl
    ...
    %                     rng(100*(sn-1)+nrr);
                        ...
                        SCORE_SS = zeros(SS,1);
                        for ss = 1:SS
                            SCORE_SS(ss) = psd_F(Dataset100(:,1,ss),...
                                Dataset200(:,1,ss));% worse than KLD
                        end
...
mytime = toc;
% save('result_pdf_acfL2_manyK.mat','SCORE_SS_NRR_NRT_SN_NL_SQ_DD',...
...
SCORE_avg = SCORE_SS_NRR_NRT_SN_NL_SQ_DD(:,1,:,:,:,:,:);
...
% acfm784 = squeeze(mean(AUC(:,:,:,:,1:5),5))
% acfs784 = squeeze(std(AUC(:,:,:,:,1:5),0,5))
% acfm768 = squeeze(mean(AUC(:,:,:,:,6:10),5))
% acfs768 = squeeze(std(AUC(:,:,:,:,6:10),0,5))
acfm784 = squeeze(mean(AUC(:,:,:,1,1:5),5))
acfs784 = squeeze(std(AUC(:,:,:,1,1:5),0,5))
acfm768 = squeeze(mean(AUC(:,:,:,1,6:10),5))
acfs768 = squeeze(std(AUC(:,:,:,1,6:10),0,5))
% 
% %sqenc=45 best performance.
% aa = mean(AUC(1,1,1,:,1:5),5);
