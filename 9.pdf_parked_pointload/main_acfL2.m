...
%                     nrt
                    ...
                    for nrr = 1:NRR %NUM RECORD reference
                        ...
                        SCORE_SS = zeros(SS,1);
                        for ss = 1:SS
                            SCORE_SS(ss) = acf_L2(Dataset100(:,1,ss),...
                                Dataset200(:,1,ss),sqenc);% worse than KLD
                        end
...
mytime = toc;
save('result_pdf_acfL2_order90.mat','SCORE_SS_NRR_NRT_SN_NL_SQ_DD',...
...
% acfm784 = squeeze(mean(AUC(:,:,:,10,1:5),5))
% acfs784 = squeeze(std(AUC(:,:,:,10,1:5),0,5))
% acfm768 = squeeze(mean(AUC(:,:,:,10,6:10),5))
% acfs768 = squeeze(std(AUC(:,:,:,10,6:10),0,5))
